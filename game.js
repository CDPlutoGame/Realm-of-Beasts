...
function playRandomMusic() {
    try {
        ...
        bgMusic.play().catch(() => console.log("Musik-Autoplay geblockt."));
    } catch(e) { console.log("Musik-Fehler"); }
}
...
window.onload = function() {
    ...
    if(savedMeta) {
        ...
        if(data.playerName) {
            document.getElementById("playerNameInput").value = data.playerName;
        }
    }
};
...
window.playerMove = function() {
    ...
    if (playerPos >= 30) {
        ...
        else if (ev === "gold") { meta.money += 15; playSound('gold'); }
...
window.buyItem = function(type) {
    ...
    if (type === 'hp') {
        ...
        if (meta.money >= cost) {
            playSound('buy');
            meta.maxHpBase += 5; meta.money -= cost; meta.hpUpgrades++;
        }
    } else if (type === 'atk') {
        let cost = (meta.atkUpgrades + 1) * 50;
        if (meta.money >= cost) {
            playSound('buy');
            meta.attackPower += 5; meta.money -= cost; meta.atkUpgrades++;
        }
    } else if (type === 'auto') {
        let cost = 1000 + (meta.autoRunLevel * 500);
        if (meta.money >= cost && meta.bossesKilled > meta.autoRunLevel) {
            playSound('buy');
            meta.money -= cost; meta.autoRunLevel++;
        }
    }
    updateUI();
};
...
function updateUI() {
    ...
    if(isGameOver) {
        arena.innerHTML = `<b style="color:red; font-size:24px;">GAME OVER</b><br><span style="font-size:12px;">Nutze den Schwarzmarkt!</span>`;
    } else if(inFight && monster) {
        arena.innerHTML = `<img src="${monster.img}" style="height:60px;"><br><b style="color:red;">${monster.name} (HP: ${monster.hp})</b>`;
    } else {
    ...
    document.getElementById("schwarzmarkt").innerHTML = `
        <div style="color:red; font-weight:bold; font-size:12px;">SCHWARZMARKT (Nur bei Game Over)</div>
    ...
    if (isGameOver) {
        ...
        actionBtn.style.background = inFight ? "red" : "#444";
        actionBtn.style.color = "white";
        actionBtn.onclick = inFight ? window.attackMonster : window.playerMove;
        
        document.getElementById("autoRunArea").innerHTML = `
            <button onclick="toggleAutoRun()" style="width:100%; margin-top:10px; padding:10px; border-radius:10px; background:${autoRunActive ? 'red' : '#333'}; color:white; font-weight:bold;">
